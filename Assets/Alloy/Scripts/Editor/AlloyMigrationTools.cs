...
public static class AlloyMigrationTools {
    ...
    [MenuItem(AlloyUtils.MenuItem + "Material Report (opened scenes)", false, 11)]
    private static void MaterialReport() {

        MaterialReportCommon(GetSceneMaterialsWithKeywords().OrderBy(m => m.name));

    }
    [MenuItem(AlloyUtils.MenuItem + "Material Report (whole project)", false, 11)]
    private static void AllMaterialReport()
    {
        MaterialReportCommon(GetAllMaterialsWithKeywords().OrderBy(m => m.name));
    }

    private static void MaterialReportCommon(IEnumerable<Material> materials)
    {
    ...
    private static void LightConverter() {
        var lights = Profile.FindObjectsOfTypeAll<Light>();
    ...
    private static IEnumerable<Material> GetSceneMaterialsWithKeywords() {
        return Profile.FindObjectsOfTypeAll<Material>().Where(m => m.shaderKeywords.Length > 0);
    }

    private static IEnumerable<Material> GetAllMaterialsWithKeywords()
    {
        return AssetDatabase.FindAssets("t: Material").Select(AssetDatabase.GUIDToAssetPath).Select(AssetDatabase.LoadAssetAtPath<Material>).Where(m => m.shaderKeywords.Length > 0);
    }
}
