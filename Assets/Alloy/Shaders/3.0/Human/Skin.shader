...
_EmissionCurve ("'Emission Curve' {Min:0, Max:2}", Float) = 0
    ...
    // Clamped decal
    [Toggle(_CLAMPEDDECAL_ON)]
    _ClampedDecal ("'Clamped Decal' {Feature:{Color:14}}", Float) = 0
    _ClampedDecalTex ("'Base Color(RGB) Mask(A)' {Visualize:{RGB, A}}", 2D) = "black" {}
    _ClampedDecalTexUV ("UV Set", Float) = 0
    [PerRendererData] _ClampedDecalSlope ("'Slope' {}", Float) = 0
    [PerRendererData] _ClampedDecalYintercept ("'Y-Intercept' {}", Float) = 0
    _ClampedDecalSpecTex ("'Metal(R) AO(G) Spec(B) Rough(A)' {Visualize:{R, G, B, A}, Parent:_MainTex}", 2D) = "white" {}
    _ClampedDecalNormalMap ("'Normals' {Visualize:{NRM}, Parent:_MainTex}", 2D) = "bump" {}
    _ClampedDecalSpecularity ("'Specularity' {Min:0, Max:1}", Float) = 1
    _ClampedDecalRoughness ("'Roughness' {Min:0, Max:1}", Float) = 1
    _ClampedDecalNormalScale ("'Normal Strength' {}", Float) = 1
    _ClampedDecalNormalWeight ("'Normal blending weight' {Min:0, Max:1}", Float) = 0.5
...
SubShader {
    ...
    Pass {
        ...
        #pragma shader_feature _DISSOLVE_ON
        #pragma shader_feature _WETNESS_ON
        #pragma shader_feature _CLAMPEDDECAL_ON
        #pragma shader_feature _WETMASKSOURCE_VERTEXCOLORALPHA
        #pragma shader_feature _ _GLOSSYREFLECTIONS_OFF
