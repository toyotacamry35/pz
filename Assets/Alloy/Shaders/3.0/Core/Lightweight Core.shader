...
Properties {
   ...
   /*// AO2
	...
	_DecalSpecTex("'Metal(R) AO(G) Spec(B) Rough(A)' {Visualize:{R, G, B, A}, Parent:_MainTex}", 2D) = "white" {}
	_DecalBumpMap("'Normals' {Visualize:{NRM}, Parent:_MainTex}", 2D) = "bump" {}
	_DecalWeight("'Weight' {Min:0, Max:1}", Float) = 1
    _DecalSpecularity ("'Specularity' {Min:0, Max:1}", Float) = 1
	_DecalRoughness("'Roughness' {Min:0, Max:1}", Float) = 1
	_DecalBumpScale("'Normal Strength' {}", Float) = 1*/
	...
	_EmissionTodSize("'Size' {Min:0, Max:1}", Float) = 0.25
	_EmissionTodMinimum("'Minimum' {Min:0, Max:1}", Float) = 0.65
    ...
    _RimPower ("'Falloff' {Min:0.01}", Float) = 4
		...
		[Toggle(_SELECTION_ON)]
	_Selection("'Selection' {Feature:{Color:11}}", Float) = 0
		[HDR]
	_SelectionColor("'Tint' {}", Color) = (1, 1, 1)
		_SelectionTex("'Effect(RGB)' {Visualize:{RGB}}", 2D) = "white" {}
	_SelectionTexVelocity("Scroll", Vector) = (0, 0, 0, 0)
		_SelectionTexUV("UV Set", Float) = 0
		[Gamma]
	_SelectionWeight("'Weight' {Min:0, Max:1}", Float) = 1
		[Gamma]
	_SelectionBias("'Fill' {Min:0, Max:1}", Float) = 0
		_SelectionPower("'Falloff' {Min:0.01}", Float) = 4
		_SelectionFadeHeight("'FadeHeight' {Min:0.01}", Float) = 2
		_SelectionFadeScale("'FadeScale' {Min:0.01}", Float) = 1
		_SelectionObjectPos("'Obj Pos' {}", Vector) = (0, 0, 0, 0)
		_MaskTex("'_MaskTex(RGB)' {Visualize:{RGB}}", 2D) = "white" {}
	_MaskTexVelocity("Scroll", Vector) = (0, 0, 0, 0)
		_MaskTexUV("UV Set", Float) = 0

		_CurvatureScale("'_CurvatureScale' {Min:0, Max:1}", Float) = 0
		_TriplanarBlendSharpnessSelect("'_TriplanarBlendSharpness' {Min:0, Max:1}", Float) = 0
		_PositionScaleSelect("'_PositionScale' {Min:0, Max:1}", Float) = 0
		_Moving("'_Moving', {Min:0, Max:1}", Float) = 0

    // Dissolve 
    [Toggle(_DISSOLVE_ON)] 
    _Dissolve ("'Dissolve' {Feature:{Color:12}}", Float) = 0
    _MaskMap ("'Mask(R)' {Visualize:{RGB}}", 2D) = "white" {}
	_MainTiling ("'Main Tiling' {Min:0, Max:15}", Float) = 0.8
    _Difference ("'Difference' {Min:0, Max:100}", Float) = 10
	_DiffNoiseTiling ("'Diff Noise Tiling' {Min:0, Max:25}", Float) = 15
	_PanSpeed ("'Pan Speed' {Min:0, Max:3}", Float) = 0
	_DiffNoiseMaskMult ("'Diff Noise Mask Mult' {Min:0, Max:15}", Float) = 0
	_DeathCutoff("'Dissolve Cutoff' {Min:0, Max:100}", Float) = 0
	_DiffNoise ("'Diff Noise(R)' {Visualize:{RGB}}", 2D) = "white" {}
	_DissolveColor ("'Dissolve Color' {}", Color) = (1,1,1)
	_DissolveGloss ("'Dissolve Gloss' {Min:0, Max:1}", Float) = 0.8	
