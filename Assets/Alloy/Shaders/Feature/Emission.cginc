    ...
    inout ASurface s)
{
    ...
    #ifdef _EMISSION_TOD_TOGGLE
        half posSin = (_EmissionTodSize * sin(_Time.w * _EmissionTodFrequency + s.positionWorld.x / _EmissionTodScale + s.positionWorld.z / _EmissionTodScale) + _EmissionTodMinimum) * _EmissionTodPower[_EmissionCurve];
		s.emissiveColor += emission * (_EmissionWeight * s.mask * posSin);
    #else
