    ...
    inout ASurface s) 
{
    ...
    half mask = s.mask * _DetailWeight;
            ...
            half3 alpha = s.vertexColor.rgb;
        #else
            half3 alpha = tex2D(_DetailMask, s.baseUv).rgb;
        #endif
    ...
    #ifndef A_DETAIL_COLOR_MAP_OFF
        float2 detailUv = A_TEX_TRANSFORM_UV_SPIN(s, _DetailAlbedoMap);
    #else
        float2 detailUv = A_TEX_TRANSFORM_UV_SPIN(s, _DetailNormalMap);
    #endif
	 
    #ifndef A_DETAIL_COLOR_MAP_OFF
        half4 detailAlbedo = tex2D(_DetailAlbedoMap, detailUv);
		half aoDetail = tex2D(_DetailNormalMap, detailUv).a;
        half3 colorScale = _DetailMode < 0.5f ? A_WHITE : unity_ColorSpaceDouble.rgb;
        s.baseColor *= aLerpWhiteTo(detailAlbedo.rgb * colorScale, mask);
		s.roughness = lerp(s.roughness, detailAlbedo.a, mask);
		s.ambientOcclusion = lerp(s.ambientOcclusion, aoDetail, mask * _DetailNormalMapScale);
    #endif
