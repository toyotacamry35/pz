...
#define A_TEX_TRANSFORM_UV_SPIN(s, name) (A_TEX_TRANSFORM_SPIN(name, A_TEX_UV(s, name)))
...
ASplat aNewSplatWorld(float3 posWorld, float3 normalWorld, ASplatContext sc, A_SAMPLER_PARAM(base), sampler2D material, sampler2D normal, half4 tint, half vertexTint, half metallic, half specularity, half specularTint, half roughness, half occlusion, half bumpScale);
