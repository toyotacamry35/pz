	...
	half bumpScale)
{
...
#ifdef DIRECTIONAL_BLEND_COLONY

	float3 _INPUT_uv = posWorld.xzy / 5;
	float _uv_Relief_z = 1.0;

	float3 triplanar_blend = abs(normalWorld);
	bool3 triplanar_flip = normalWorld.xyz<0;
	...
	if (triplanar_blend_flag)
	{
		...
		{
			c = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsAlbedo, float3(uvTRI1.xy, _NormalSwitch.x)).rgb;
			col += triplanar_blend.x * c;

			normals_combined = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsNormal, float3(uvTRI1.xy, _NormalSwitch.x)).rg;
			nA.xy = normals_combined * 2 - 1;
		...
		{
			c = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsAlbedo, float3(uvTRI2.xy, _NormalSwitch.x)).rgb;
			col += triplanar_blend.y * c;

			normals_combined = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsNormal, float3(uvTRI2.xy, _NormalSwitch.x)).rg;
			nB.xy = normals_combined * 2 - 1;
		...
		{
			c = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsAlbedo, float3(uvTRI3.xy, _NormalSwitch.x)).rgb;
			col += triplanar_blend.z * c;

			normals_combined = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsNormal, float3(uvTRI3.xy, _NormalSwitch.x)).rg;
			nC.xy = normals_combined * 2 - 1;
			nC.y = triplanar_flip.z ? -nC.y : nC.y;
			nC.z = sqrt(1 - saturate(dot(nC.xy, nC.xy)));
			
		}
		n = (triplanar_blend.x * nA + triplanar_blend.y * nB + triplanar_blend.z * nC);

	}
	else
	{
		c = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsAlbedo, float3(uvTRI.xy, _NormalSwitch.x)).rgb;
		col += c;
		normals_combined = UNITY_SAMPLE_TEX2DARRAY(TerrainSplatsNormal, float3(uvTRI.xy, _NormalSwitch.x)).rg;
		n.xy = normals_combined  * 2 - 1;
	...
	sp.baseUv = posWorld.xz / 5;
	sp.normal = n;// UnpackScaleNormal(tex2D(normal, sp.baseUv), -bumpScale);
	sp.material0 = float4(col,1);// tex2D(base, sp.baseUv);
	sp.material1 = tex2D(material, sp.baseUv);
	sp.A_AMBIENT_OCCLUSION = aOcclusionStrength(sp.A_AMBIENT_OCCLUSION, occlusion);
#endif
...
//#else
//	sp.A_METALLIC = 0.0h;
//	sp.A_AMBIENT_OCCLUSION = 0.0h;
//	sp.A_SPECULARITY = 0.0h;
//	sp.A_ROUGHNESS = 1.0h;
//	sp.A_OPACITY = 1.0h;
//#endif
