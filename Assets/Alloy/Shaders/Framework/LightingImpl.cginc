    ...
    half vertexTint)
{
	...
	s.mask *= weight * mask * s.ambientOcclusion;
}

void aBlendRangeMaskWorld(
	inout ASurface s,
	half mask,
	half weight,
	half cutoff,
	...
	half vertexTint)
{
	...
	mask *= weight * mask;
	//s.mask = saturate(mask + (1 - lerp(1, s.ambientOcclusion, _OcclusionStrength)));
	s.mask = saturate(mask + (1 - pow(s.ambientOcclusion, _OcclusionStrength)));
    #ifdef _COATING_MASK
	    //A_SAMPLER_2D(_CoatingMask);
	    float2 coatingMaskUV = A_TEX_TRANSFORM_SPIN(_CoatingMask, s.baseUv.xy);
		//A_TEX_TRANSFORM_UV_SPIN(s, _CoatingMask);// A_TEX_TRANSFORM_UV_SPIN(s, _CoatingMask);
	    half4 ctex = tex2D(_CoatingMask, coatingMaskUV);
	    s.mask *= saturate(ctex.r + (1 -_CoatingMaskPower));
    #endif
#endif
}
...
void aUpdateViewDataBlend(
    inout ASurface s)
{
#if defined(A_NORMAL_WORLD_ON) && defined(A_VIEW_DIR_WORLD_ON)
    // Area lights need this to be per-pixel.
    #ifdef A_REFLECTION_VECTOR_WORLD_ON
        s.reflectionVectorWorld = reflect(-s.viewDirWorld, s.objectNormalWorld);
    #endif

    // Skip re-calculating world normals in some cases.
    #ifndef A_VIEW_DIR_TANGENT_ON
        s.NdotV = aDotClamp(s.objectNormalWorld, s.viewDirWorld);
    #else
        s.NdotV = aDotClamp(s.normalTangent, s.viewDirTangent);
    #endif
