    ...
    public class Triangulator
    {
        ...
        LinkedList<Vertex> GenerateVertexList(Polygon polygon)
        {
            ...
            foreach (HoleData holeData in sortedHoleData)
            {
                ...
                {
                    if (nodePotentiallyInTriangle == null)
                        continue;
                ...
                if (nextVertexPos.HasValue && validBridgeNodeOnHull != null)
                {
                    bool isConvex = IsConvex(holeData.bridgePoint, validBridgeNodeOnHull.Value.position, nextVertexPos.Value);
