         ...
         for (int i = 0; i < src.Count; ++i)
         {
            try
            {
              ...
              if (e.smoothness != null)
                     MergeInChannel(diffuseHeightTex, (int)TextureArrayConfig.TextureChannel.A, e.smoothness, smoothChannel, e.isRoughness);
               ...
               if (e.height != null)
               {
                  MergeInChannel(normalSAOTex, (int)TextureArrayConfig.TextureChannel.R, e.height, heightChannel, true, diffuseIsLinear);
               }
               ...
               {
                  EditorUtility.CompressTexture(normalSAOTex, GetTextureFormat(), UnityEditor.TextureCompressionQuality.Normal);
               }

               if (cfg.diffuseCompression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(diffuseHeightTex, GetTextureFormat(), UnityEditor.TextureCompressionQuality.Normal);
               }

               if (antiTileTex != null && cfg.antiTileCompression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(antiTileTex, GetTextureFormat(), UnityEditor.TextureCompressionQuality.Normal);
               }

               if (cfg.emisMetalArray && !cfg.IsAdvancedDetail() && emisTex != null && cfg.emisCompression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(emisTex, GetTextureFormat(), UnityEditor.TextureCompressionQuality.Normal);
               }
#else
                if (cfg.normalCompression != TextureArrayConfig.Compression.Uncompressed)
                {
