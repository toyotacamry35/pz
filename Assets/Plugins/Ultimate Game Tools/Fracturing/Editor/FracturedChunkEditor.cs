...
#if AK_MOD
    SerializedProperty PropIsROOTSupportChunk;
#endif
...
#if AK_MOD
        PropIsROOTSupportChunk    = serializedObject.FindProperty("IsROOTSupportChunk");
#endif
    }
...
#if AK_MOD
            if (!PropIsSupportChunk.boolValue)
            {
                PropIsROOTSupportChunk.boolValue = false;
            }
#endif
        }
...
#if AK_MOD
        EditorGUI.BeginChangeCheck();
        PropIsROOTSupportChunk.boolValue = EditorGUILayout.Toggle(new GUIContent("Is Root Support Chunk", "Whether this chunk is the most root one - will be destructed last \n #Note: Multiple chunks could 've this flag On, BUT these chunks should not be vertically interconnected among themselves, \n 'cos `DestructController` logic will leave only 1 chunk with this flag On at its start. \n Multiple chunks with this flag are used only to randomize the very last chunk among instances."), PropIsROOTSupportChunk.boolValue);
        if (EditorGUI.EndChangeCheck())
        {
            if (PropIsROOTSupportChunk.boolValue)
            {
                PropIsSupportChunk.boolValue = true;
            }
        }
#endif
