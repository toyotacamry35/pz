...
namespace UltimateFracturing
{
    static partial class Fracturer
    {
                                                    ...
                                                    List<int>[] aListIndicesPosInOut, List<VertexData> listVertexDataPosInOut, List<int>[] aListIndicesNegInOut, List<VertexData> listVertexDataNegInOut)
        {
            ...
            List<List<Poly2TriFrac.Point2D>>      listlistCapPoints        = new List<List<Poly2TriFrac.Point2D>>();
            List<List<Poly2TriFrac.PolygonPoint>> listlistCapPolygonPoints = new List<List<Poly2TriFrac.PolygonPoint>>();
            List<Poly2TriFrac.Polygon>            listCapPolygons          = new List<Poly2TriFrac.Polygon>();

            for(int i = 0; i < listlistPointsConstrainedDelaunay.Count; i++)
            {
                List<Poly2TriFrac.Point2D>      listCapPoints         = new List<Poly2TriFrac.Point2D>();
                List<Poly2TriFrac.PolygonPoint> listCapPolygonsPoints = new List<Poly2TriFrac.PolygonPoint>();
                ...
                foreach(Vector3 v3Vertex in listlistPointsConstrainedDelaunay[i])
                {
                    ...
                    listCapPoints.Add        (new Poly2TriFrac.Point2D     (v3VertexInPlane.x, v3VertexInPlane.z));
                    listCapPolygonsPoints.Add(new Poly2TriFrac.PolygonPoint(v3VertexInPlane.x, v3VertexInPlane.z));
                }

                listlistCapPoints.Add(listCapPoints);
                listlistCapPolygonPoints.Add(listCapPolygonsPoints);
                listCapPolygons.Add(new Poly2TriFrac.Polygon(listCapPolygonsPoints));
            }
            ...
            int nSuperPolygon = -1;
            Poly2TriFrac.Polygon polygonContainer = null;
            ...
            if(bForceVertexSoup == false)
            {
                for(int i = 0; i < listlistCapPolygonPoints.Count; i++)
                {
                    for(int j = 0; j < listlistCapPolygonPoints.Count; j++)
                    {
                        ...
                        {
                            if(Poly2TriFrac.PolygonUtil.PolygonContainsPolygon(listlistCapPoints[i], listCapPolygons[i].Bounds, listlistCapPoints[j], listCapPolygons[j].Bounds, true))
                            {
                                nSuperPolygon = i;
                                break;
                            }
                            else if(Poly2TriFrac.PolygonUtil.PolygonContainsPolygon(listlistCapPoints[j], listCapPolygons[j].Bounds, listlistCapPoints[i], listCapPolygons[i].Bounds, true))
                            {
                                nSuperPolygon = j;
                                break;
                            }
            ...
            if(polygonContainer != null && bForceVertexSoup == false)
            {
                ...
                try
                {
                    Poly2TriFrac.P2T.Triangulate(polygonContainer);
            ...
            if(bTriangulatedWithHoles == false)
            {
                if(bForceVertexSoup)
                {
                    ...
                    List<Poly2TriFrac.TriangulationPoint> listPoints = new List<Poly2TriFrac.TriangulationPoint>();

                    if(listlistCapPolygonPoints.Count > 0)
                    {
                        foreach(Poly2TriFrac.PolygonPoint polyPoint in listlistCapPolygonPoints[0])
                        {
                        ...
                        try
                        {
                            ...
                            {
                                Poly2TriFrac.PointSet ps = new Poly2TriFrac.PointSet(listPoints);
                                Poly2TriFrac.P2T.Triangulate(ps);
                ...
                else
                {
                    ...
                    foreach(List<Poly2TriFrac.PolygonPoint> listPolyPoints in listlistCapPolygonPoints)
                    {
                        IList<Poly2TriFrac.DelaunayTriangle> listTriangles = null;

                        Poly2TriFrac.Polygon polygon = null;

                        try
                        {
                            if(listPolyPoints.Count >= 3)
                            {
                                polygon = new Poly2TriFrac.Polygon(listPolyPoints);
                                Poly2TriFrac.P2T.Triangulate(polygon);
                        ...
                        if(listTriangles == null)
                        {
                            ...
                            foreach(Poly2TriFrac.PolygonPoint polyPoint in listPolyPoints)
                            {
                                listPoints.Add(polyPoint);
                            }
        ...
        static void CreateIndexedMesh(IList<Poly2TriFrac.DelaunayTriangle> listTriangles, List<Vector3> listVerticesOut, List<int> listIndicesOut, Matrix4x4 mtxTransform, bool bTransform)
        {
            ...
            foreach(Poly2TriFrac.DelaunayTriangle triangle in listTriangles)
            {
                for(int i = 0; i < 3; i++)
                {
                    bool bFound = false;
                    ...
                    Poly2TriFrac.TriangulationPoint point = triangle.PointCWFrom(triangle.Points[i]);
                    v3Vertex.x = point.Xf;
                    v3Vertex.z = point.Yf;
