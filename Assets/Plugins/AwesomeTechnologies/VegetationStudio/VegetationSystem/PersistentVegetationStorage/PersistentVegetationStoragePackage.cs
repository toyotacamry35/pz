...
namespace AwesomeTechnologies.Vegetation.PersistentStorage
{
    ...
    public class PersistentVegetationCell
    {
        ...
        public void RemoveVegetationItemInstance(string vegetationItemID, Vector3 origin, Vector3 position, float minimumDistance)
        {
            PersistentVegetationInfo persistentVegetationInfo = GetPersistentVegetationInfo(vegetationItemID);
            if (persistentVegetationInfo == null) return;

            var ray = new Ray(origin, position - origin);

            for (int i = persistentVegetationInfo.VegetationItemList.Count - 1; i >= 0; i--)
            {
                if (DistanceToLine(ray, persistentVegetationInfo.VegetationItemList[i].Position) < minimumDistance)
                //if (Vector3.Distance(persistentVegetationInfo.VegetationItemList[i].Position, position) < minimumDistance)
                {
    ...
    public class PersistentVegetationStoragePackage : ScriptableObject
    {
        ...
        public void RemoveVegetationItemInstance(int cellIndex, string vegetationItemID, Vector3 origin, Vector3 position, float minimumDistance)
        {
            if (PersistentVegetationCellList.Count > cellIndex)
            {
                PersistentVegetationCellList[cellIndex].RemoveVegetationItemInstance(vegetationItemID, origin, position, minimumDistance);
            }
