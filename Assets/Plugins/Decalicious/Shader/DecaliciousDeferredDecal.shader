...
Shader "Decalicious/Deferred Decal"
{
	Properties
	{
		...
		_SmoothnessTex ("Roughness", 2D) = "white" {}
	...
	SubShader
	{
...
#ifndef UNITY_HDR_ON
				color.rgb = exp2(-max(color.rgb, 0));
#endif
		...
		Pass
		{
			...
			void frag(v2f i, out float4 outSpecSmoothness : SV_Target0, out float4 outNormal : SV_Target1)
			{
				...
				outSpecSmoothness = float4(lerp(specSmoothness.xyz, specularVal.rgb * _SpecularMultiplier.rgb, mask * specularVal.a * _SpecularMultiplier.a),
										   lerp(specSmoothness.a, (1-smoothnessVal.r) * _SmoothnessMultiplier, mask * smoothnessVal.a * _SpecularMultiplier.a));
			}
			ENDCG
		}
	}
