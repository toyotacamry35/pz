...
//			}
			...
			fixed4 frag (v2f IN) : SV_Target
			{
				...
				half3 colorGamma = LinearToGammaSpace(color);
				half colorAlpha = pow(color.a + 0.01, 1.6) / 0.9;
				
				if (_UseAlphaClip)
					clip (colorAlpha - 0.001);
				if(_LineWeight>0){
					half l = (_LineWeight+1/_PixelWorldScale)/2;
					colorAlpha *= saturate((l-distance(visible(IN.texcoord,_Radius),l))*_PixelWorldScale);
				}
				else{
					colorAlpha *= saturate(visible(IN.texcoord,_Radius)*_PixelWorldScale);
				}
				return half4(colorGamma,colorAlpha);
			}
			ENDCG
		}
	}
