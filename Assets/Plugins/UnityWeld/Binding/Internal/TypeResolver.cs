...
namespace UnityWeld.Binding.Internal
{
    ...
    public static class TypeResolver
    {
        ...
        {
            var assemblies =
                AppDomain.CurrentDomain.GetAssembliesSafe()
        ...
        private static IEnumerable<MethodInfo> GetPublicMethods(Type type)
        {
            if (!type.IsInterface)
            {
                return type.GetMethodsSafe(BindingFlags.Public | BindingFlags.Instance);
            }

            return (new[] { type })
                .Concat(type.GetInterfaces())
                .SelectMany(i => i.GetMethodsSafe(BindingFlags.Public | BindingFlags.Instance));
        }
