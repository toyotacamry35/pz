...
namespace GeneratedCode.DeltaObjects
{
    ...
    {
        [SharedCode.EntitySystem.RpcClassHashAttribute(1458487018)]
        internal static class ReceiveFuncs
        {
            [SharedCode.EntitySystem.RpcMethodReceiverFunc(0, "IFounderPack_RefreshStatus_List_Message")]
            internal static async void IFounderPack_RefreshStatus_List_Message_Func(IEntitiesRepository _repository, INetworkProxy networkProxy, Guid callback, byte[] __data__, int __offset__, PropertyAddress __propertyAddress__, IDeltaObject __targetObj__, Guid _, Guid __migrationId__)
            {
                try
                {
                    var serializer = ((IEntitiesRepositoryExtension)_repository).Serializer;
                    var packs = serializer.Deserialize<System.Collections.Generic.List<string>>(__data__, ref __offset__);
                    using (GeneratedCode.Manual.AsyncStack.AsyncStackIsolator.IsolateContext())
                    {
                        ...
                        using (await __task)
                        {
                            var __entity__ = __targetObj__.To<GeneratedCode.DeltaObjects.IFounderPack>();
                            await __entity__.RefreshStatus(packs);
                        }
                    }
                }
                catch (Exception e)
