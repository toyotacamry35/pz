...
namespace GeneratedCode.DeltaObjects
{
    ...
    {
        [SharedCode.EntitySystem.RpcClassHashAttribute(1213603192)]
        internal static class ReceiveFuncs
        {
            [SharedCode.EntitySystem.RpcMethodReceiverFunc(0, "ILogableEntity_SetCurveLoggerEnable_Boolean_Message")]
            internal static async void ILogableEntity_SetCurveLoggerEnable_Boolean_Message_Func(IEntitiesRepository _repository, INetworkProxy networkProxy, Guid callback, byte[] __data__, int __offset__, PropertyAddress __propertyAddress__, IDeltaObject __targetObj__, Guid _, Guid __migrationId__)
            {
                try
                {
                    var serializer = ((IEntitiesRepositoryExtension)_repository).Serializer;
                    var val = serializer.Deserialize<bool>(__data__, ref __offset__);
                    using (GeneratedCode.Manual.AsyncStack.AsyncStackIsolator.IsolateContext())
                    {
                        ...
                        using (await __task)
                        {
                            var __entity__ = __targetObj__.To<SharedCode.Aspects.Utils.ILogableEntity>();
                            await __entity__.SetCurveLoggerEnable(val);
                        }
                    }
                }
                catch (Exception e)
