...
{
    public class EntityUpdateStatistics : CoreBaseStatistic
    {
        ...
        private static readonly NLog.Logger _overallStatisticsLog = LogManager.GetLogger("Telemetry-EntityUpdateStatistics");

        private int useSecondDict = 0;
        private readonly MetricsStorage<int, LabeledMetrics, Metrics> _metricStorage;

        public EntityUpdateStatistics()
        {
            var metrics = new Metrics(Prometheus.Metrics.CreateSummary("entity_update_sent", "entity_update_sent", "entitytid"),
                Prometheus.Metrics.CreateSummary("entity_update_received", "entity_update_received", "entitytid"));
        ...
        public void EntityUpdateSend(int typeId, long bytesCount)
        {
            EntityUpdateSendInternal(typeId, bytesCount);
        }

        [Conditional("ENABLE_NETWORK_STATISTICS")]
        private void EntityUpdateSendInternal(int typeId, long bytesCount)
        {
        ...
        public void EntityUpdateReceived(int typeId, long bytesCount)
        {
            EntityUpdateReceivedInternal(typeId, bytesCount);
        }

        [Conditional("ENABLE_NETWORK_STATISTICS")]
        private void EntityUpdateReceivedInternal(int typeId, long bytesCount)
        {
            var metrics = _metricStorage.GetOrCreate(typeId);
        ...
        class StatInfo
        {
            ...
            public List<EntityUpdateStatistics.Stat> TopSendedBytes { get; set; }
            public List<EntityUpdateStatistics.Stat> TopReceivedBytes { get; set; }
        }
