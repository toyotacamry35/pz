...
namespace GeneratedCode.DeltaObjects
{
    ...
    public interface IPlaceObjectCounter : IQuestCounter
    {
    ...
    {
        private PlaceObjectCounterDef Def => (PlaceObjectCounterDef)CounterDef;
        ...
        private async Task Subscribe(IEntitiesRepository repository, bool subscribe)
        {
            using (var wrapper = await repository.Get(parentEntity.TypeId, parentEntity.Id))
            {
                ...
                if (subscribe)
                    (await entity.StatisticEngine.GetStatisticRouter<PlaceObjectEventArgs>()).RoutedEvent += StatisticsCounter_RoutedEvent;
                else
                    (await entity.StatisticEngine.GetStatisticRouter<PlaceObjectEventArgs>()).RoutedEvent -= StatisticsCounter_RoutedEvent;
        ...
        private async Task StatisticsCounter_RoutedEvent(PlaceObjectEventArgs arg)
        {
            if (_counterCompleted)
                return;
            ...
            {
                if (!Def.HaveTargets || Def.AllTargets.Contains(arg.PlacedObject))
                    await IncreaseItems();
            }
        }
        ...
        public override string ToString()
        {
            ...
            sb.AppendFormat("PlaceObjectCounter: Def = {0}, Objects = ", Def.____GetDebugShortName());
