...
namespace GeneratedCode.DeltaObjects
{
    ...
    public interface IMobKillsCounter : IQuestCounter
    {
    ...
    {
        private MobKillsCounterDef Def => (MobKillsCounterDef)CounterDef;
        ...
        private async Task Subscribe(IEntitiesRepository repository, bool subscribe)
        {
            using (var wrapper = await repository.Get(parentEntity.TypeId, parentEntity.Id))
            {
                ...
                if (subscribe)
                    (await entity.StatisticEngine.GetStatisticRouter<KillMortalObjectEventArgs>()).RoutedEvent += StatisticsCounter_RoutedEvent;
                else
                    (await entity.StatisticEngine.GetStatisticRouter<KillMortalObjectEventArgs>()).RoutedEvent -= StatisticsCounter_RoutedEvent;
        ...
        private async Task StatisticsCounter_RoutedEvent(KillMortalObjectEventArgs arg)
        {
            if (_counterCompleted)
                return;
            ...
            {
                if (!Def.HaveTargets || Def.AllTargets.Contains(arg.MortalObjectDef))
                    await IncreaseItems();
            }
        }
        ...
        public override string ToString()
        {
            ...
            sb.AppendFormat("KillsCounter: Def = {0}, Mobs = ", Def.____GetDebugShortName());
