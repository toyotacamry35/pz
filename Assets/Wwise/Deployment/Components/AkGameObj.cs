...
public class AkGameObj : UnityEngine.MonoBehaviour
{
    ...
    private bool _isOffsetUsed = true;

    private const float MAXIMUM_SQR_DISTANCE = 350f;
    ...
    private void Awake()
    {
        ...
        if (Register() == AKRESULT.AK_Success)
        {
            ...
            if (isEnvironmentAware)
            {
                ...
                m_envData.UpdateAuxSend(gameObject, _position);
            }
    ...
    private void SetCachedParameters()
    {
        ...
        if (m_positionOffsetData == null || m_positionOffsetData.positionOffset == Vector3.zero)
        {
            m_positionOffsetData = null;
            _isOffsetUsed = false;
        }
    }
    ...
    private static float NonPreciseCalculation(Vector3 a, Vector3 b)
    {
        var num1 = a.x - b.x;
        var num2 = a.y - b.y;
        var num3 = a.z - b.z;
    ...
    private void Update()
    {
        ...
        //Не используем в звуке его направление
        /*
        ...
        var position = GetPosition();
        if (m_posData.position == position)
            return;
    ...
    {
        return _isOffsetUsed ? _position + _transform.rotation * m_positionOffsetData.positionOffset : _position;
    }
                ...
                for (var ii = 0; ii < AK_NUM_LISTENERS; ++ii)
                {
                    if (listeners[ii] != null && listeners[ii].Count > 1)
                    {
                        UnityEngine.Debug.LogWarning(
                            "WwiseUnity: Multiple listeners <" +
                            listeners[ii].Count +
                            "> with same listenerId <" +
                            ii +
