    ...
    public class GameObjectBrushEditor : EditorWindow
    {
        ...
        void OnGUI()
        {
            if (!Application.isPlaying)
            {
                ...
                if (GUILayout.Button("Reload All Brushes"))
                {
                    BrushCollection.ClearAllBrushCollectionGUIDsCache();
                }
                if (GUILayout.Button("Save All Brushes To Disk"))
                {
                    realSave = true;
                }
                ...
                foreach (BrushObject brObj in brushes.brushes)
                {
                    ...
                    var preview = AssetPreview.GetAssetPreview(brObj.brushObject);

                    if (preview == null)
                    {
                        var previewGiver = brObj.brushObject.GetComponent<IGOBrushPreviewImageGiver>();
                        if (previewGiver != null)
                            preview = previewGiver.GetPreview;

                    }
                    GUIContent btnContent = new GUIContent(preview, brObj.brushObject.name);
                ...
                if (realSave || GUI.changed && (brushes != null))
                {
                    if (realSave)
                    {
                        realSave = false;
                ...
                for (int i = 0; i < spawnCount; i++)
                {
                    ...
                    if (brush.brushObject != null && IsOpen)
                    {
                        ...
                        if (Physics.Raycast(ray, out hit))
                        {
                            ...
                            if (obj != null)
                            {
                                var afterBrushCallback = obj.GetComponent<IBrushExtender>();
                                if (afterBrushCallback != null)
                                    afterBrushCallback.AfterBrushPlacement();
                            }
                        }
                    }
                }
