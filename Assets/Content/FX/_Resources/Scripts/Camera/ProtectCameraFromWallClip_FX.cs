...
namespace Assets.Content.FX.Scenes
{
    public class ProtectCameraFromWallClip_FX : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (sphereCastRadius > 70)
                Logger.IfError()?.Message($"ProtectCameraFromWallClip_FX radius is too big {sphereCastRadius}").Write();
            ...
            if (initialIntersect)
            {
                ...
                if (m_OriginalDist - sphereCastRadius > 150)
                    Logger.IfError()?.Message($"ProtectCameraFromWallClip_FX rayLength is too big {m_OriginalDist - sphereCastRadius}").Write();
